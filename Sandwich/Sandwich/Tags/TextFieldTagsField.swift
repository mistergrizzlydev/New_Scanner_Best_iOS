...
public struct TextFieldTagAcceptOption: OptionSet {
    ...
    public static let `return` = TextFieldTagAcceptOption(rawValue: 1 << 0)
    public static let  comma   = TextFieldTagAcceptOption(rawValue: 1 << 1)
    public static let  space   = TextFieldTagAcceptOption(rawValue: 1 << 2)
    public static let  minus   = TextFieldTagAcceptOption(rawValue: 1 << 3)
}

@IBDesignable
open class TextFieldTagsField: UIScrollView {
    ...
    open var restrictTyping: Bool = false
    ...
    open var acceptTagOption: TextFieldTagAcceptOption = .return
     ...
     */
    open var onVerifyTag: ((TextFieldTagsField, _ text: String) -> Bool)?
     ...
     */
    open var onDidChangeHeightTo: ((TextFieldTagsField, _ height: CGFloat) -> Void)?
    ...
    open func addTags(_ tags: [TextFieldTag]) {
        tags.forEach { addTag($0) }
    }

    open func addTag(_ tag: String) {
        addTag(TextFieldTag(tag))
    }

    open func addTag(_ tag: TextFieldTag) {
        ...
        let tagView = TextFieldTagView(tag: tag)
    ...
    open func removeTag(_ tag: String) {
        removeTag(TextFieldTag(tag))
    }

    open func removeTag(_ tag: TextFieldTag) {
    ...
    @discardableResult
    open func tokenizeTextFieldText() -> TextFieldTag? {
    ...
    open func selectTagView(_ tagView: TextFieldTagView, animated: Bool = false) {
        if self.readOnly {
            return
        }
...
extension TextFieldTagsField {
    ...
    func inputAccessoryView(view: UIView) {
        inputFieldAccessoryView?.addSubview(view)
    }
}
...
//        self.layer.cornerRadius = 12
    ...
    fileprivate func enumerateItemRects(layoutWidth: CGFloat,
                                        using closure: (_ tagView: TextFieldTagView?, _ tagRect: CGRect?,
                                                        _ textFieldRect: CGRect?) -> Void) {
        if layoutWidth == 0 {
            return
        }
    ...
    fileprivate func repositionViews() {
        ...
        enumerateItemRects(layoutWidth: self.bounds.width) { (tagView: TextFieldTagView?, tagRect: CGRect?, textFieldRect: CGRect?) in
...
extension TextFieldTagsField: UITextFieldDelegate {
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        guard !restrictTyping else {
            return false
        }
//        guard tags.contains(.init(string)) else {
//            return false
//        }
        
        if acceptTagOption.contains(.minus) && string == "-" && onShouldAcceptTag?(self) ?? true {
            tokenizeTextFieldText()
            return false
        }
...
extension TextFieldTagsField {
    public static func == (lhs: UITextField, rhs: TextFieldTagsField) -> Bool {
